...
func (b *XADatasourceResource) GetKeeper() map[string]Holdable {
	return b.keeper
}

func (b *XADatasourceResource) SetKeeper(keeper map[string]Holdable) {
	b.keeper = keeper
}
...
func (b *XADatasourceResource) Hold(key string, value Holdable) (interface{}, error) {
	if value.IsHeld() {
		var x = b.keeper[key]
		if x != value {
...
func (b *XADatasourceResource) Release(key string, value Holdable) (Holdable, error) {
	if value.IsHeld() {
		var x = b.keeper[key]
		if x != value {
			return nil, fmt.Errorf("something wrong with keeper, keeping[%v] but[%v] is also kept with the same key[%v]", x, value, key)
...
func (b *XADatasourceResource) Lookup(key string) (*ConnectionProxyXA, bool) {
	holdable, ok := b.keeper[key]
	connectionProxyXA, ok := holdable.(*ConnectionProxyXA)
	if !ok {
		return nil, false
	}
...
func (b *XADatasourceResource) ConnectionForXA(ctx context.Context, xaXid XAXid) (*ConnectionProxyXA, error) {
	...
	xaConnection, err := CreateXAConnection(ctx, conn, b.GetDbType())
	if err != nil {
		return nil, err
	}

	connectionProxyXA, err := NewConnectionProxyXA(conn, xaConnection, b, xaXid.String())
	if err != nil {
