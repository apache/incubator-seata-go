...
// buildUndoSQL
func (m *MySQLUndoInsertExecutor) buildUndoSQL(dbType types.DBType, sqlUndoLog impl.SQLUndoLog) (string, error) {
	afterImage := sqlUndoLog.AfterImage
...
func (m *MySQLUndoInsertExecutor) generateDeleteSql(
	...
	rows []types.RowImage,
	dbType types.DBType,
	sqlUndoLog impl.SQLUndoLog) (string, error) {

	colImages, err := m.BaseExecutor.GetOrderedPkList(image, rows[0], dbType)
	if err != nil {
		return "", err
	}
