...
func TestBuildSelectSQLByMultiUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			query, args, err := executor.buildBeforeImageSQL(updateStmts, tt.sourceQueryArgs, types.TableMeta{})
	...
	_, _, err = executor.buildBeforeImageSQL(updateStmts, sourceQueryArgs, types.TableMeta{})
	assert.Error(t, err)
...
func TestBuildSelectSQLByMultiUpdateAllColumns(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			query, args, err := executor.buildBeforeImageSQL(updateStmts, tt.sourceQueryArgs, meta)
			assert.NoError(t, err)
			assert.Equal(t, tt.expectQuery, query)
			assert.Equal(t, tt.expectQueryArgs, args)
		})
	...
	executor := NewMultiUpdateExecutor(c, &types.ExecContext{Values: sourceQueryArgs, NamedValues: util.ValueToNamedValue(sourceQueryArgs)}, []exec.SQLHook{})
	_, _, err = executor.buildBeforeImageSQL(updateStmts, sourceQueryArgs, meta)
	assert.Error(t, err)
	assert.Equal(t, err.Error(), "multi update SQL with orderBy condition is not support yet")
}
