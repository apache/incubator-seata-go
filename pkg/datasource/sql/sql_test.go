...
func Test_SQLOpen(t *testing.T) {
	...
	log.Info("begin test")
	var err error
	db, err = sql.Open(SeataMySQLDriver, "root:12345678@tcp(127.0.0.1:3306)/seata_client?multiStatements=true")
	if err != nil {
		t.Fatal(err)
	}
	...
	txInvoker := func(prefix string, offset, total int) {
		...
		res, err := tx.Exec("update foo set name=?,id=id where id > ?", prefix, 1)
		if err != nil {
			t.Fatal(err)
		}
		fmt.Sprintln(res)
