...
import (
	...
	"seata.apache.org/seata-go/v2/pkg/saga/runtime"
	...
	tmctx "seata.apache.org/seata-go/v2/pkg/tm"
...
func (p ProcessCtrlStateMachineEngine) startInternal(ctx context.Context, stateMachineName string, tenantId string,
	...
	stateMachineInstance.Context = contextMap
	...
	if stateMachineInstance.ID == "" {
		stateMachineInstance.ID = p.StateMachineConfig.SeqGenerator().GenerateId(constant.SeqEntityStateMachineInst, "")
	}

	var eventBus process_ctrl.EventBus
	if async {
		eventBus = p.StateMachineConfig.AsyncEventBus()
	} else {
		eventBus = p.StateMachineConfig.SyncEventBus()
	}

	err = eventBus.Run(ctx, processContext)
	if err != nil {
		return nil, err
	}
...
func (p ProcessCtrlStateMachineEngine) forwardInternal(ctx context.Context, stateMachineInstId string,
	...
	stateMachineInstance.Context = concurrentContextVariables
	...
	if lastForwardState.TypeName == constant.StateTypeSubStateMachine && lastForwardState.CompensationStatus() != statelang.SU {
		context.SetVariable(constant.VarNameIsForSubStatMachineForward, true)
	}

	if lastForwardState.Status != statelang.SU {
		lastForwardState.IgnoreStatus = true
	}
	...
	stateMachineInstance.Status = statelang.RU
	...
	if async {
		if err := p.StateMachineConfig.AsyncEventBus().Run(ctx, context); err != nil {
			return nil, err
		}
	} else {
		if err := p.StateMachineConfig.SyncEventBus().Run(ctx, context); err != nil {
			return nil, err
		}
	}
...
func (p ProcessCtrlStateMachineEngine) findOutLastForwardStateInstance(stateInstanceList []*statelang.StateInstance) (*statelang.StateInstance, error) {
	...
	for i := len(stateInstanceList) - 1; i >= 0; i-- {
		...
		if !stateInstance.IsForCompensation() {
			...
			if stateInstance.TypeName == constant.StateTypeSubStateMachine {
...
func (p ProcessCtrlStateMachineEngine) createMachineInstance(stateMachineName string, tenantId string, businessKey string, startParams map[string]interface{}) (*statelang.StateMachineInstance, error) {
	...
	if startParams != nil {
		...
		if parentRaw, exists := startParams[constant.VarNameParentId]; exists {
			...
			stateMachineInstance.ParentID = parentId
	...
	now := time.Now()
	stateMachineInstance.StartedTime = now
	stateMachineInstance.UpdatedTime = now
	return stateMachineInstance, nil
}
...
func (p ProcessCtrlStateMachineEngine) compensateInternal(ctx context.Context, stateMachineInstId string, replaceParams map[string]any,
	...
	if async {
		err := p.StateMachineConfig.AsyncEventBus().Run(ctx, context)
		if err != nil {
			return nil, err
		}
	} else {
		err := p.StateMachineConfig.SyncEventBus().Run(ctx, context)
		if err != nil {
			return nil, err
		}
	}
