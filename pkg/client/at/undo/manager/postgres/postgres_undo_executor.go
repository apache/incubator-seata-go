...
func DeleteBuildUndoSql(undoLog undo.SqlUndoLog) string {
	...
	return fmt.Sprintf(InsertSqlTemplate, stringUtil.Escape(undoLog.TableName, "`"), insertColumns, insertValues)
}
...
func InsertBuildUndoSql(undoLog undo.SqlUndoLog) string {
	...
	pkField := row.PrimaryKeys()[0]
	return fmt.Sprintf(DeleteSqlTemplate, stringUtil.Escape(undoLog.TableName, "`"), pkField.Name)
}
...
func NewPostgresUndoExecutor(undoLog undo.SqlUndoLog) PostgresUndoExecutor {
	return PostgresUndoExecutor{sqlUndoLog: undoLog}
}

func (executor PostgresUndoExecutor) Execute(tx *sql.Tx) error {
...
func (executor PostgresUndoExecutor) dataValidationAndGoOn(tx *sql.Tx) (bool, error) {
...
func (executor PostgresUndoExecutor) queryCurrentRecords(tx *sql.Tx) (*schema.TableRecords, error) {
	...
	inCondition := sql2.AppendInParamPostgres(len(pkValues))
	selectSql := fmt.Sprintf(SelectSqlTemplate, b.String(), stringUtil.Escape(tableMeta.TableName, "`"), pkName, inCondition)
