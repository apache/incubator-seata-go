...
func TestLoadPath(t *testing.T) {
	...
	assert.NotNil(t, cfg.ClientConfig.LoadBalanceConfig)
	assert.Equal(t, "RandomLoadBalance", cfg.ClientConfig.LoadBalanceConfig.Type)
	assert.Equal(t, 10, cfg.ClientConfig.LoadBalanceConfig.VirtualNodes)

	assert.NotNil(t, cfg.RemotingConfig)
	assert.NotNil(t, cfg.RemotingConfig.SessionConfig)
	assert.Equal(t, 0, cfg.RemotingConfig.ReconnectInterval)
	assert.Equal(t, 1, cfg.RemotingConfig.ConnectionNum)
	assert.Equal(t, false, cfg.RemotingConfig.SessionConfig.CompressEncoding)
	assert.Equal(t, true, cfg.RemotingConfig.SessionConfig.TCPNoDelay)
	assert.Equal(t, true, cfg.RemotingConfig.SessionConfig.TCPKeepAlive)
	assert.Equal(t, time.Minute*2, cfg.RemotingConfig.SessionConfig.KeepAlivePeriod)
	assert.Equal(t, 262144, cfg.RemotingConfig.SessionConfig.TCPRBufSize)
	assert.Equal(t, 65536, cfg.RemotingConfig.SessionConfig.TCPWBufSize)
	assert.Equal(t, time.Second, cfg.RemotingConfig.SessionConfig.TCPReadTimeout)
	assert.Equal(t, time.Second*5, cfg.RemotingConfig.SessionConfig.TCPWriteTimeout)
	assert.Equal(t, time.Second, cfg.RemotingConfig.SessionConfig.WaitTimeout)
	assert.Equal(t, 16498688, cfg.RemotingConfig.SessionConfig.MaxMsgLen)
	assert.Equal(t, "client_test", cfg.RemotingConfig.SessionConfig.SessionName)
	assert.Equal(t, time.Second, cfg.RemotingConfig.SessionConfig.CronPeriod)
...
func TestLoadJson(t *testing.T) {
	...
	assert.NotNil(t, cfg.ClientConfig.LoadBalanceConfig)
	assert.Equal(t, "XID", cfg.ClientConfig.LoadBalanceConfig.Type)
	assert.Equal(t, 1, cfg.ClientConfig.LoadBalanceConfig.VirtualNodes)

	assert.NotNil(t, cfg.RemotingConfig)
	assert.NotNil(t, cfg.RemotingConfig.SessionConfig)
	assert.Equal(t, 1, cfg.RemotingConfig.ReconnectInterval)
	assert.Equal(t, 10, cfg.RemotingConfig.ConnectionNum)
	assert.Equal(t, true, cfg.RemotingConfig.SessionConfig.CompressEncoding)
	assert.Equal(t, false, cfg.RemotingConfig.SessionConfig.TCPNoDelay)
	assert.Equal(t, false, cfg.RemotingConfig.SessionConfig.TCPKeepAlive)
	assert.Equal(t, time.Minute*2, cfg.RemotingConfig.SessionConfig.KeepAlivePeriod)
	assert.Equal(t, 261120, cfg.RemotingConfig.SessionConfig.TCPRBufSize)
	assert.Equal(t, 32768, cfg.RemotingConfig.SessionConfig.TCPWBufSize)
	assert.Equal(t, time.Second*2, cfg.RemotingConfig.SessionConfig.TCPReadTimeout)
	assert.Equal(t, time.Second*8, cfg.RemotingConfig.SessionConfig.TCPWriteTimeout)
	assert.Equal(t, time.Second*2, cfg.RemotingConfig.SessionConfig.WaitTimeout)
	assert.Equal(t, 261120, cfg.RemotingConfig.SessionConfig.MaxMsgLen)
	assert.Equal(t, "client_test", cfg.RemotingConfig.SessionConfig.SessionName)
	assert.Equal(t, time.Second*2, cfg.RemotingConfig.SessionConfig.CronPeriod)

	assert.NotNil(t, cfg.RemotingConfig.StreamConfig)
	assert.Equal(t, 1048576, cfg.RemotingConfig.StreamConfig.MaxRecvMsgSize)
	assert.Equal(t, 1048576, cfg.RemotingConfig.StreamConfig.MaxSendMsgSize)
	assert.Equal(t, time.Second*3, cfg.RemotingConfig.StreamConfig.KeepAliveTime)
	assert.Equal(t, time.Second*1, cfg.RemotingConfig.StreamConfig.KeepAliveTimeout)
	assert.Equal(t, false, cfg.RemotingConfig.StreamConfig.PermitWithoutStream)
	assert.Equal(t, time.Second*10, cfg.RemotingConfig.StreamConfig.HeartbeatInterval)
	assert.Equal(t, time.Second*50, cfg.RemotingConfig.StreamConfig.DialTimeout)
