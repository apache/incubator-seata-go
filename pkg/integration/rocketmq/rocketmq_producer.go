...
func (p *RocketMQProducer) SendSync(ctx context.Context, msg interface{}) (interface{}, error) {
	mqMsg, ok := msg.(*primitive.Message)
	if !ok {
		return nil, fmt.Errorf("message must be *primitive.Message, got %T", msg)
	}

	// Use type assertion to access the underlying producer
	type producerWithSync interface {
		SendSync(ctx context.Context, msgs ...*primitive.Message) (*primitive.SendResult, error)
	}

	if syncProducer, ok := p.transProducer.(producerWithSync); ok {
		result, err := syncProducer.SendSync(ctx, mqMsg)
		if err != nil {
			return nil, fmt.Errorf("send message sync failed: %w", err)
		}
	...
	return nil, fmt.Errorf("transaction producer does not support SendSync")
}
...
// GetGlobalStatus queries the actual global transaction status from TC server
func (c *DefaultGlobalStatusChecker) GetGlobalStatus(ctx context.Context, xid string) (int32, error) {
	if xid == "" {
		return int32(message.GlobalStatusUnKnown), fmt.Errorf("xid is empty")
	}

	// Create request to query global transaction status
	...
	// Send sync request to TC server
	res, err := getty.GetGettyRemotingClient().SendSyncRequest(req)
	if err != nil {
		log.Errorf("query global status failed, xid=%s, error=%v", xid, err)
		return int32(message.GlobalStatusUnKnown), fmt.Errorf("query global status failed: %w", err)
	}

	if res == nil {
		log.Errorf("global status response is nil, xid=%s", xid)
		return int32(message.GlobalStatusUnKnown), fmt.Errorf("global status response is nil")
	}

	statusResp, ok := res.(message.GlobalStatusResponse)
	if !ok {
		log.Errorf("global status response type assertion failed, xid=%s, type=%T", xid, res)
		return int32(message.GlobalStatusUnKnown), fmt.Errorf("invalid response type: %T", res)
	}

	if statusResp.ResultCode == message.ResultCodeFailed {
		log.Errorf("query global status result code is failed, xid=%s, msg=%s", xid, statusResp.Msg)
		return int32(message.GlobalStatusUnKnown), fmt.Errorf("query failed: %s", statusResp.Msg)
	}

	log.Infof("query global status success, xid=%s, status=%v", xid, statusResp.GlobalStatus)
	return int32(statusResp.GlobalStatus), nil
}
