...
func TestSeataTransactionListenerCheckLocalTransaction_StatusMapping(t *testing.T) {
	tests := []struct {
		...
		{name: "commit failed", globalStatus: message.GlobalStatusCommitFailed, expected: primitive.UnknowState},
...
func newTransactionMessageExt(xid, branchID string) *primitive.MessageExt {
	msgExt := &primitive.MessageExt{Message: primitive.Message{Topic: "topic-test", Body: []byte("hello")}}
	if xid != "" {
